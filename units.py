...
class MilitaryUnit:
    ...
    def _get_reachable_priority_targets(self, enemies): # how to select target
        ...
        return list(filter(lambda unit: unit.__class__ in self.__get_targets(), targets_in_range))
...
class AirForce(MilitaryUnit):
    def __init__(self, name: str, longtitude: int, latitude: int, altitude: int, attack_range: int, delta_altitude: int, min_altitude: int, max_altitude: int) -> None:
