...
class MilitaryUnit:
    def __init__(self, name: str, longtitude: int, latitude: int, attack_range: int, accuracy: float, vulnerability: float) -> None:
        self.name = name
    ...
    def __sub__(self, instance):
        return ((self.latitude - instance.latitude)**2 + (self.longtitude - instance.longtitude)**2)**0.5
    ...
    def __get_euclidian_distance(self, unit):
        return ((self.longtitude - unit.longtitude)**2+(self.longtitude - unit.longtitude)**2)**0.5
    
    def __scan_targets(self, enemies):
        return list(filter(lambda enemy: self.__get_euclidian_distance(self, enemy) <= self.attack_range, enemies))
...
class AirForce(MilitaryUnit):
    def __init__(self, name: str, longtitude: int, latitude: int, altitude: int, attack_range: int, min_altitude: int, max_altitude: int, accuracy: float, vulnerability: float) -> None:
        ...
        super().__init__(name, longtitude, latitude, attack_range, accuracy, vulnerability)
...
class Stormtrooper(Troops):
    def __init__(self, name, longtitude, latitude) -> None:
        self.actions = ['move_north', 'move_south', 'move_west, move_east', 'attack', 'follow_vehicle', 'leave_vehicle']
...
class Fighter(AirForce):
    def __init__(self, name, longtitude, latitude) -> None:
        self.targets = [Bomber, Fighter, Helicopter,]
        super().__init__(name, 
                         longtitude, 
                         latitude, 
                         altitude=0, 
